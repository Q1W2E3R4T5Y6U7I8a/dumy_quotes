...
class ImagePool:
    ...
    def get(self):
        ...
        if len(avail) == len(self.images): self.used.clear()
...
def main():
    CONFIG = {'json': "quotes.json", 'images': "downloaded_images", 
              'out': "output_quotes", 'variants': 3, 'total_display': 365}
    
    try:
        with open(CONFIG['json'], 'r', encoding='utf-8') as f:
            quotes = json.load(f)
    except: print(f"❌ Error loading {CONFIG['json']}"); return
    
    os.makedirs(CONFIG['out'], exist_ok=True)
    pool = ImagePool(CONFIG['images'])
    
    if not pool.images: print(f"❌ No images in '{CONFIG['images']}'"); return
    
    total = len(quotes)
    print("═"*60 + f"\n🎨 QUOTE GENERATOR | {total} quotes × {CONFIG['variants']} variants\n" + "═"*60)
    
    success, failed = 0, 0
    start = datetime.now()
    
    for idx, q in enumerate(quotes, 1):
        preview = q.get('quote', '')[:50] + '...'
        print(f"\n📝 [{idx}/{total}] {preview}")
        
        for v in range(1, CONFIG['variants']+1):
            img = pool.get()
            if not img: print(f"   ⚠️ No image"); failed+=1; continue
            
            name = f"quote_{idx:03d}" + (f"_v{v}" if CONFIG['variants']>1 else "") + ".jpg"
            out = os.path.join(CONFIG['out'], name)
            
            try:
                create_quote(q, img[0], out, idx, CONFIG['total_display'])
                success+=1
                print(f"   ✓ {name} ({img[1]})")
